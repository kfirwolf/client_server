...
int buddy_alloc_destroy(buddy_allocator_t *b_alloc) {
    ...
    // First, tear down the ID‐pool you used for node indices
    id_pool_destroy(b_alloc->max_nodes_id_pool);

    // Finally free the big block that holds your
    // buddy_allocator_t, free_list[], and node_pool[]
    free(b_alloc);
...
int buddy_alloc_allocate(buddy_allocator_t *b_alloc, uint32_t block_size, block_t *block_out) {
    ...
    /*
        ...
        2. calc free list index from power of 2 block size 
        3. 
