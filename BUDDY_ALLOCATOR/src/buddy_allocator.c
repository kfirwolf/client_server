...
struct block_mem_pool {
    struct block *node_pool;
    id_pool_t *max_nodes_id_pool;
    uint32_t total_max_nodes_size;
};

struct buddy_allocator {
    uint32_t min_block_size; 
    uint32_t num_of_levels;
    struct block_mem_pool block_pool;
...
int buddy_alloc_create(buddy_alloc_cfg_t *b_cfg, buddy_allocator_t **b_alloc) {
    ...
    b_allocator->block_pool.node_pool = calloc(1, max_nodes_size);

    if (unlikely(b_allocator->block_pool.node_pool == NULL)) {
    ...
    //setting block pool
    b_allocator->block_pool.total_max_nodes_size = max_nodes;

    //setting id pool
    client_pool_cfg.capacity = b_allocator->block_pool.total_max_nodes_size;
    client_pool_cfg.id_start_offset = 0;

    rc = id_pool_create(&(b_allocator->block_pool.max_nodes_id_pool), &client_pool_cfg);
    if (unlikely(rc != 0)) {
...
int buddy_alloc_allocate(buddy_allocator_t *b_alloc, uint32_t block_size, block_t *block_out) {
    ...
    if (current_block != NULL) {
        block_out->offset = current_block->offset;
        block_out->size = split_size;
        rc = linked_list_extract_block_by_offset(b_alloc, level, current_block->offset, NULL);
        if (unlikely(rc != 0)) {
            return rc;            
        }
    ...
    while (++level <= original_block_level) { 
        split_size = split_size >> 1; //2 times smaller size
        buddy_offset = (block_offset ^ split_size);
        linked_list_add_block_as_head(b_alloc, level, buddy_offset);
    }
