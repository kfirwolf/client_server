...
static int linked_list_add_block(buddy_alloc_t *b_alloc, struct buddy_alloc_block *input_block) {
    ...
    if (b_alloc->free_list[input_block->level].head != NULL) {
...
static int remove_buddy_block(buddy_alloc_t *b_alloc, const uint32_t *offset, const int *level, const uint32_t *block_size, uint32_t *buddy_offset, bool *found_buddy) {
    int rc = 0;
    *found_buddy = false;
...
static inline int find_non_empty_level(const buddy_alloc_t *b_alloc, uint32_t start_level, uint32_t *found_level) {
...
static int split_block_down(buddy_alloc_t *b_alloc, uint32_t src_level, uint32_t target_level, struct buddy_alloc_block **target_blk) {
    ...
    // remove the src block (not from mem pool) from the free list, we will split it into smaller blocks and add them to the free list, except for the target block that we will return to the user
    ...
    b_size = get_block_size(src_level, b_alloc->min_block_size, b_alloc->num_of_levels);
