...
int store_mngr_create(store_mngr_t **s_mngr, store_mngr_cfg_t* s_mngr_cfg) {
    ...
    if (s_mngr_cfg->buddy_min_block_size == 0) {
        store_manager->buddy_min_block_size = MIN_MEM_POOL_BLOCK_SIZE_IN_BYTES;
    }
    else {
        store_manager->buddy_min_block_size = s_mngr_cfg->buddy_min_block_size;
    }
    ...
    buddy_alloc_cfg.min_block_size = store_manager->buddy_min_block_size;
...
int store_mngr_block_alloc(store_mngr_t *s_mngr, uint32_t block_size, store_mngr_block_t *b_handler) {
    ...
    rc = buddy_alloc_allocate(s_mngr->buddy_alloc, block_size, &b_handler->block);
    if (unlikely(rc != 0)) {
